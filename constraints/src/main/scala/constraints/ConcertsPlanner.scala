...
object ConcertsPlanner {
  ...
  def plan(preferences: Map[Band, List[Slot]]): Option[Map[Band, Slot]] = {
    ...
    val desirableSlots: Seq[Formula] = bands.map{ b => preferences(b).map(propVariables(b,_)).foldLeft[Formula](false)(_ || _)}
                                                //preferences(b).map(propVariables(b,_)) is List of PropVar

    //A set of constraints ensuring that a band gets at most one slot
    val eachBandPlaysOnce: Seq[Formula] = bands.map {
      b => {
        for { s1 <- slots
              s2 <- slots.dropWhile(_ != s1)
              if (s1 != s2) } yield !propVariables(b, s1) || !propVariables(b, s2)
        }.foldLeft[Formula](true)(_ && _)
    }
  
    //A set of constraints ensuring that each slot is used at most once
    val eachSlotUsedOnce: Seq[Formula] = slots.map {
      s => {
        for{b1 <- bands
            b2 <- bands.dropWhile(_!=b1)
            if(b1 != b2)} yield !propVariables(b1, s) || !propVariables(b2, s)
      }.foldLeft[Formula](true)(_ && _)
    }
   ...
   */
  def getAllSlots(preferences: Map[Band, List[Slot]]): Set[Slot] = preferences.toList.flatMap(_._2).toSet
