...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
      def timesAcc(chars: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] = chars match {
        case Nil => acc
        case x :: xs => timesAcc(xs.filter(_ != x), (x, chars.count(_ == x)) :: acc)
      }
      timesAcc(chars, List())
    }
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
      case Nil => Nil
      case x :: Nil => trees
      case x :: y :: xs => (makeCodeTree(x,y) :: trees.tail.tail).sortBy(weight(_))
    }
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
		...
		def dec(remaining: CodeTree, bits: List[Bit]): List[Char] = remaining match {
        case Leaf(c, _) => if (bits.isEmpty) List(c)
						else c :: dec(tree, bits)
        case Fork(left, right, _, _) => if (bits.head == 0) dec(left, bits.tail)
										else dec(right, bits.tail)
      }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def encodeChar(tree:  CodeTree)(c: Char): List[Bit] = tree match {
        case Leaf(_, _) => List()
        case Fork(left, right, _, _) => if(chars(left).contains(c)) 0 :: encodeChar(left)(c)
   ...
   */
    def convert(tree: CodeTree): CodeTable = tree match {
      case Leaf(char,w) => List( (char,List()) )
   ...
   */
    def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = a.map(codeGen(_)(0)) ::: b.map(codeGen(_)(1))

    def codeGen(node: (Char,List[Bit]))(b: Bit): (Char,List[Bit]) = (node._1, b :: node._2)
